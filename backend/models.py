...
class ASTNode(db.Model):
    ...
    node_type = db.Column(db.String, nullable=False)  # "operator", "operand", or "constant"
    operator = db.Column(db.String, nullable=True)     # "AND", "OR", etc.
    ...
    # Relationships
    left = db.relationship('ASTNode', foreign_keys=[left_node], remote_side=[id])
    right = db.relationship('ASTNode', foreign_keys=[right_node], remote_side=[id])
